...
/**
 * 感谢 Tom Wu : http://www-cs-students.stanford.edu/~tjw/jsbn/
 *
 * 基础 JavaScript 椭圆曲线实现
 * 从 BouncyCastle 的 Java EC 代码松散移植
 * 目前仅实现 Fp 曲线
 */
...
class ECFieldElementFp {
  /**
   * 构造函数
   * @param {BigInteger} q - 域大小
   * @param {BigInteger} x - 元素值
   */
  constructor(q, x) {
    this.x = x;
    this.q = q;
  ...
  /**
   * 判断两个元素是否相等
   * @param {ECFieldElementFp} other - 另一个元素
  ...
  /**
   * 取反
   * @returns {ECFieldElementFp} 取反后的元素
   */
  negate() {
  ...
  /**
   * 相加
   * @param {ECFieldElementFp} b - 另一个元素
   * @returns {ECFieldElementFp} 相加后的元素
   */
  add(b) {
   ...
   * 相减
   * @param {ECFieldElementFp} b - 另一个元素
   * @returns {ECFieldElementFp} 相减后的元素
   */
  subtract(b) {
   ...
   * 相乘
   * @param {ECFieldElementFp} b - 另一个元素
   * @returns {ECFieldElementFp} 相乘后的元素
   */
  multiply(b) {
   ...
   * 相除
   * @param {ECFieldElementFp} b - 另一个元素
   * @returns {ECFieldElementFp} 相除后的元素
   */
  divide(b) {
  ...
  /**
   * 平方
   * @returns {ECFieldElementFp} 平方后的元素
   */
  square() {
...
class ECPointFp {
  /**
   * 构造函数
   * @param {ECCurveFp} curve - 椭圆曲线
   * @param {ECFieldElementFp} x - x 坐标
   * @param {ECFieldElementFp} y - y 坐标
   * @param {BigInteger} z - z 坐标（射影坐标系）
   */
   ...
   * z3 = λ10 * λ8
   * 
   * @param {ECPointFp} b - 另一个点
   * @returns {ECPointFp} 相加后的点
   */
  add(b) {
  ...
  /**
   * 点倍乘
   * @param {BigInteger} k - 倍数
   * @returns {ECPointFp} 倍乘后的点
   */
  multiply(k) {
    if (this.isInfinity()) return this;
...
class ECCurveFp {
   ...
   * 构造函数
   * @param {BigInteger} q - 域大小
   * @param {BigInteger} a - 系数 a
   * @param {BigInteger} b - 系数 b
   */
  constructor(q, a, b) {
    this.q = q;
   ...
   * 生成椭圆曲线域元素
   * @param {BigInteger} x - 元素值
   * @returns {ECFieldElementFp} 椭圆曲线域元素
   */
  fromBigInteger(x) {
  ...
  /**
   * 解析十六进制串为椭圆曲线点
   * @param {string} s - 十六进制串
   * @returns {ECPointFp|null} 椭圆曲线点或 null
   */
  decodePointHex(s) {
    switch (parseInt(s.substr(0, 2), 16)) {
      ...
      case 3:
        ...
        // 算出结果二进制最后 1 位不等于第 1 个字节减 2 则取反
